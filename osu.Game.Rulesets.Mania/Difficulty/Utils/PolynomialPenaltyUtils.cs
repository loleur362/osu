...
namespace osu.Game.Rulesets.Mania.Difficulty.Utils
{
    ...
    public static class PolynomialPenaltyUtils
    {
        ...
        /// <summary>
        /// Creates a polynomial curve that maps score loss to accuracy penalties.
        /// Used to smoothly interpolate between accuracies, with 0 score loss fixed to 0% penalty, and all score loss fixed to 100% penalty.
        /// </summary>
        /// <param name="scoreLoss">
        /// A dictionary of score losses, with keys representing skill proportions and values representing the score loss a player would achieve at that skill proportion.
        /// See comment on <see cref="SKILL_PROPORTIONS"/> if you want to use custom skill proportions.
        /// </param>
        public static QuarticCoefficients GetPenaltyCoefficients(Dictionary<double, double> scoreLoss)
        {
            double endPoint = scoreLoss.Values.Max();

            double[] sortedSkillProportions = scoreLoss.Keys.OrderByDescending(k => k).ToArray();
            ...
            for (int row = 0; row < matrix.Length; row++)
            {
                for (int column = 0; column < matrix[row].Length; column++)
                {
                    ...
                    double missCountAtSkill = scoreLoss[skillProportion];
            ...
            return new QuarticCoefficients(coefficients[0], coefficients[1], coefficients[2], coefficients[3]);
        }

        public readonly record struct QuarticCoefficients(double A, double B, double C, double D)
        {
            public double[] ToArray() => new[] { A, B, C, D };
        }
        ...
        /// <param name="totalScoreLoss">The number of misses the player got</param>
        /// <returns>A value between 0 and 1 representing the penalty percentage (0 = no penalty, 1 = full penalty)</returns>
        public static double GetPenaltyAt(QuarticCoefficients coefficients, double totalScoreLoss)
        {
